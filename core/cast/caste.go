...
func TimeE(i interface{}) (tim time.Time, err error) {
	return TimeInDefaultLocationE(i, time.UTC)
}

// TimeInDefaultLocationE casts an empty interface to time.Time,
...
func TimeInDefaultLocationE(i interface{}, location *time.Location) (tim time.Time, err error) {
	...
	case time.Time:
		return v, nil
	case string:
		return StringDateInDefaultLocation(v, location)
	case int:
...
// SliceE casts an interface to a []interface{} type.
...
// StringDate attempts to parse a string into a time.Time type using a
// predefined list of formats.  If no suitable format is found, an error is
// returned.
func StringDate(s string) (time.Time, error) {
	return parseDateWith(s, time.UTC, timeFormats)
}

// StringDateInDefaultLocation casts an empty interface to a time.Time,
// interpreting inputs without a timezone to be in the given location,
// or the local timezone if nil.
func StringDateInDefaultLocation(s string, location *time.Location) (time.Time, error) {
...
// jsonStringObject attempts to unmarshall a string as JSON into
// the object passed as pointer.
func jsonStringObject(s string, v interface{}) error {
	data := []byte(s)
